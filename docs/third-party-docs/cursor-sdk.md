...
> **Source:** [https://cursor.com/docs/sdk/python](https://cursor.com/docs/sdk/python)
> **Fetched:** 2026-06-13
> **Package:** `cursor-sdk` (pip) — local mirror for offline reference.
...
To target a self-hosted pool, set `env` on `CloudAgentOptions`:
...
Use `ModelSelection.params` to pass per-model options such as reasoning effort or max mode. Parameter IDs and values vary by model. Use `Cursor.models.list()` to discover supported parameters and preset variants for your account.
...
Use `Cursor.models.list()` to discover the parameter IDs and preset variants for a given model.
...
| Member | Description |
| --- | --- |
| `agent_id` | Stable agent identifier. `agent- ` for local, `bc- ` for cloud. |
...
| --- | --- | --- | --- |
| `model` | `str | ModelSelection | Mapping[str, Any]` | Required for local; cloud falls back to the server-resolved default | Model to use. See `ModelSelection`. |
...
| Property | Type | Default | Description |
| --- | --- | --- | --- |
| `env` | `CloudEnvironment | Mapping[str, Any]` | `{ type: "cloud" }` | Execution environment. `cloud` uses Cursor-hosted VMs; `pool` and `machine` target a self-hosted pool. |
