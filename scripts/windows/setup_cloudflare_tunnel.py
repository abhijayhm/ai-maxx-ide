...
def load_env() -> dict[str, str]:
    ...
    if not path.exists():
        ...
        raise SystemExit(
            f"Missing {path}\n"
            f"Copy {sample} to .env and set SERVER_DOMAIN, SSH_DOMAIN, TUNNEL_NAME, LOCAL_SERVER_PORT."
        )
...
def build_ingress_rules(env: dict[str, str]) -> list[tuple[str, str]]:
    ...
    ssh_domain = require_env(env, "SSH_DOMAIN")
    port = env_int(env, "LOCAL_SERVER_PORT")

    rules = [
        (server_domain, f"http://127.0.0.1:{port}"),
...
def ensure_windows_ssh_server():
    if not is_admin():
        raise RuntimeError(
            "OpenSSH Server setup requires Administrator. "
            "Re-run setup_cloudflare_tunnel.bat as Admin, or set SKIP_SSH_SETUP=true."
        )
    ps = r'''
Write-Host "[ssh] Checking OpenSSH Server capability..."
$server = Get-WindowsCapability -Online | Where-Object Name -like 'OpenSSH.Server*'
if ($server.State -ne 'Installed') {
  Write-Host "[ssh] Installing OpenSSH Server (can take 5-15 min, no further output until done)..."
  Add-WindowsCapability -Online -Name OpenSSH.Server~~~~0.0.1.0
  Write-Host "[ssh] OpenSSH Server installed."
} else {
  Write-Host "[ssh] OpenSSH Server already installed."
}
Write-Host "[ssh] Starting sshd..."
Start-Service sshd
Write-Host "[ssh] Setting sshd to start automatically..."
Set-Service -Name sshd -StartupType Automatic
if (!(Get-NetFirewallRule -Name "OpenSSH-Server-In-TCP" -ErrorAction SilentlyContinue)) {
  Write-Host "[ssh] Adding firewall rule for port 22..."
  New-NetFirewallRule -Name 'OpenSSH-Server-In-TCP' -DisplayName 'OpenSSH Server (sshd)' -Enabled True -Direction Inbound -Protocol TCP -Action Allow -LocalPort 22
}
Write-Host "[ssh] Done. Service status:"
...
def main():
    print("Cloudflare Tunnel + SSH setup (ai-maxx-ide)")
    ...
    ssh_domain = require_env(env, "SSH_DOMAIN")
    server_domain = require_env(env, "SERVER_DOMAIN")
    ssh_user = env.get("SSH_USERNAME", "").strip()
    ...
    print(f"SSH:         {ssh_domain} -> localhost:22")

    ensure_cloudflared()

    if is_windows():
        if env_bool(env, "SKIP_SSH_SETUP"):
            print("Skipping SSH server setup (SKIP_SSH_SETUP=true).")
        else:
            ensure_ssh_server()
    elif not port_open_local(22):
        print("Warning: SSH port 22 is not listening. Configure openssh-server before using SSH tunnel.")
    ...
    if not env_bool(env, "SKIP_FLASK_SMOKE_TEST", default=True):
        write_python_app(port, APP_NAME)

    write_ssh_client_config(ssh_domain, ssh_user)
    ...
    for hostname, service in ingress_rules:
        print(f"  - {hostname} ({service})")
    print("\nNext steps:")
    print(f"1) Start Django on 127.0.0.1:{port}")
    print(f"2) cloudflared tunnel run {tunnel_name}")
    if ssh_user:
        print(f"3) SSH: ssh {ssh_domain}")
